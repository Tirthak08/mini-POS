...
export async function changePin(req, res) {
  ...
  if (!business) throw ApiError.unauthorized('This business no longer exists');
  /**
   * 400, not 401. A wrong `currentPin` here is a mistyped FIELD, not a bad
   * session -- the token that carried this request is perfectly valid. Sending
   * 401 made the app's interceptor do the only thing 401 can honestly mean:
   * discard the token and bounce to the sign-in screen. So a shopkeeper who
   * fat-fingered their old PIN was thrown out of the app and told their session
   * had expired. 401 stays reserved for the auth middleware.
   */
  if (!(await business.verifyPin(req.body.currentPin))) {
