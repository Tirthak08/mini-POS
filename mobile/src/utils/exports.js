...
export async function exportExcel(exportPayload) {
  const { orders = [], items = [], totals, business, range } = exportPayload;
...
function buildReportHtml({ business, range, totals, summary, orders = [], topProducts = [] }) {
...
<body>
  ...
  <div class="kpis">
    ...
    ${kpi('Profit', formatINR(summary?.profit ?? totals?.profit ?? 0), 'pos')}
  ...
  <div class="foot">Generated ${esc(formatDate(new Date(), { withTime: true }))} &middot; Mini POS</div>
</body></html>`;
}
