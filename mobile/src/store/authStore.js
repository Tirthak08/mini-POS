...
export const useAuthStore = create()(
  persist(
    (set, get) => ({
      ...
      /**
       * Renaming the shop. The name is not just a label -- it is half of the
       * sign-in credential, so the store has to hold the server's version of it
       * rather than the text that was typed. The server trims, slugs and may
       * reject the name as taken; keeping its answer is what stops the header
       * showing a name the operator can no longer sign in with.
       */
      renameBusiness: async (name) => {
        try {
          const res = await authApi.renameBusiness(name);
          set({ business: res.business });
          return { ok: true, business: res.business };
        } catch (err) {
          return { ok: false, error: err };
        }
      },
