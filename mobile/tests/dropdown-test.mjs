...
/**
 * Switching language is no longer one tap in the header -- the EN/HI/GU
 * segmented control was replaced by Settings > Language, a radio list written
 * in each language's own script.
 *
 * Which means the route to it is itself translated, so the walk needs to know
 * what language the UI is CURRENTLY in: the Settings entry is reached by the
 * shop's name (never translated), the Language row by the label in the OLD
 * language, and the two Back arrows by the label in the NEW one, since the
 * radio takes effect immediately.
 */
const LANG_LABEL = { EN: 'English', HI: 'हिन्दी', GU: 'ગુજરાતી' };
let uiLang = 'EN';

const setLang = async (code) => {
  const from = LOCALES[uiLang];
  const to = LOCALES[code];

  await page.getByRole('button', { name: new RegExp(shop) }).last().click();
  await page.waitForTimeout(1300);

  // The row's accessible name is "Language, English" -- label plus its current
  // value -- so it is matched as a prefix rather than exactly.
  const langRow = page.getByRole('button', { name: new RegExp(`^${from.settings.language}`) }).last();
  await langRow.waitFor({ state: 'visible', timeout: 15000 });
  await langRow.click();
  await page.waitForTimeout(1100);

  const radio = page.getByRole('radio', { name: LANG_LABEL[code], exact: true }).last();
  await radio.waitFor({ state: 'visible', timeout: 15000 });
  await radio.click();
  await page.waitForTimeout(1400);

  // Language -> Settings -> back to the tab the walk was on.
  for (let i = 0; i < 2; i += 1) {
    await page.getByRole('button', { name: to.common.back, exact: true }).last().click();
